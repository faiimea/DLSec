...
class WaNet(base.BackdoorAttack):
    ...
    def __init__(self, tag: str = 'CustomModel', device: str = 'cpu', model=None, dataset=None, poison_rate: float = 0.05, lr: float = 0.1, target_label=2, epochs: int = 20, batch_size: int = 64, optimizer: str = 'sgd', criterion=None, local_state_path: str = None, s: float = 0.5, k: int = 4,
        ...
        if local_state_path is not None and os.path.isfile("./LocalModels/" + local_state_path[:-4] + "WAgrid.pt"):
            self.WAgrid=torch.load("./LocalModels/" + local_state_path[:-4] + "WAgrid.pt")
        else:
