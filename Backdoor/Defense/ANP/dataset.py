...
def generate_trigger(trigger_type,path=None):
    ...
    elif trigger_type == 'gaussian_noise' and path is not None:
        trigger = Image.open(path)

        # 调整触发器的大小
        trigger = trigger.resize((32, 32))
        trigger = trigger.convert('RGB')
        # 将触发器转换为 numpy 数组
        pattern = np.array(trigger)
...
def add_trigger_cifar(data_set, trigger_type, poison_rate, poison_target, trigger_alpha=1.0,path=None):
    """
    ...
    """
    pattern, mask = generate_trigger(trigger_type=trigger_type,path=path)
