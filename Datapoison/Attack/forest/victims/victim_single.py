...
class _VictimSingle(_VictimBase):
    ...
    def initialize(self, pretrain=False, seed=None):
        if self.args['random_seed'] is None:
            if seed is None:
                self.model_init_seed = np.random.randint(0, 2**32 - 1, dtype=np.int64)
            else:
                self.model_init_seed = seed
        else:
            self.model_init_seed = self.args['random_seed']
        ...
        print(f'model initialized with random key {self.model_init_seed}.')
    ...
    def _iterate(self, kettle, poison_delta, max_epoch=200, pretraining_phase=False):
        """Validate a given poison by training the model and checking target accuracy."""
        stats = defaultdict(list)

        single_setup = (self.model, self.optimizer, self.scheduler)
        for self.epoch in range(max_epoch):
            print(f'Epoch:{self.epoch + 1}/{max_epoch}')
