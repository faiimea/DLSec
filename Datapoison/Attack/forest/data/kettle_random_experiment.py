...
class KettleRandom(_Kettle):
    ...
    def random_construction(self):
        ...
        if self.args['poisonkey'] is None:
            self.init_seed = np.random.randint(0, 2**32 - 1, dtype=np.int64)
        else:
    ...
    def _parse_threats_randomly(self):
        ...
        if self.args['targets'] < 1:
            poison_setup = dict(target_num=0, poison_class=np.random.randint(num_classes), target_class=None,
                                intended_class=[np.random.randint(num_classes)])
            print('Number of targets set to 0.')
            return poison_setup

        poison_class = intended_class[0]
        poison_setup = dict(target_num=self.args['targets'],
